...
%include "extern.inc"    ; extern declarations for every GTK/Adw/gettext call used below
...
section .rodata
    ...
    comments_str  db "A pure x86-64 assembly recreation of classic Notepad for Linux, built directly on the GTK4 and libadwaita C ABI (no C glue code). Created by Tiglate Pileser III (tiglate) with AI assistance.", 0  ; i18n: the app/developer name and the URL above are proper nouns, deliberately NOT translated -- only this long-form description is
...
on_about_activate:
    ...
    lea  rdi, [rel comments_str]  ; arg1 = msgid -- long-form description, incl. the AI-assisted-authorship note
    CCALL gettext                   ; char *gettext(const char *msgid) -- rax = the translated string, or msgid itself unchanged if there's no translation loaded; borrowed, never freed by us
    mov  rsi, rax                    ; arg2 = the (possibly translated) comments text
